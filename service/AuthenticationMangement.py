...
class Authentication:
    ...
    def get_acess_token(refresh_token):
        ...
        try:
            ...
            print(err)
            raise HTTPException(
                status_code=401, detail="Token is expired. Please update your token.",
            )
        except jwt.InvalidSignatureError as err:
            print(err)
            raise HTTPException(
    ...
    def login(username, password):
        ...
        user_check = [
            user for user in Authentication.userDB.find({"username": username})
        ]
        ...
        if len(user_check) == 1:
            ...
            if isVerified == True:
                ...
                raise HTTPException(
                    status_code=401, detail="Please verify email first !"
                )
        else:
    ...
    def generate_confrim_email(email, uuid_key, timeout):
        ...
        confrim_email_token = jwt.encode(payload, ENV.SECERET_EMAIL_KEY)
    ...
    def send_verify_email(username, email, uuid_key, timeout=10):
        ...
        confrim_email_token = Authentication.generate_confrim_email(
            email, uuid_key, timeout
        )
        confrim_email_link = (
            f"{ENV.ROOT_URL}/api/v1/confrim/email?verify_token={confrim_email_token}"
        )
        ...
        r = requests.post(
            ...
            data={
                "from": f"MDNEX Support Admin <{ENV.MAILGUN_DOMAIN}>",
                "to": f"{username} <{email}>",
